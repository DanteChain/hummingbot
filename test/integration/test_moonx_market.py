...
from hummingbot.core.event.events import MarketEvent, TradeType, OrderType, TradeFee, BuyOrderCompletedEvent, \
    OrderFilledEvent, BuyOrderCreatedEvent, SellOrderCompletedEvent, OrderCancelledEvent, SellOrderCreatedEvent
...
class MoonxMarketTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.market: MoonxMarket = MoonxMarket(
            conf.moonx_business_num,
            conf.moonx_api_secret,
            trading_pairs=[PAIR]
        )

        cls.count = 0

        print("Initializing Moonx market... this will take about a minute. ")
    ...
    async def wait_til_ready(cls):
        while True:
            ...
            if cls.market.ready:
                ...
                await cls.clock.run_til(next_iteration)
            cls.count += 1
            await asyncio.sleep(1.0)

    def setUp(self):
        self.db_path: str = realpath(join(__file__, "../moonx_test.sqlite"))
        try:
    ...
    def test_get_fee(self):
        limit_fee: TradeFee = self.market.get_fee(BASE_ASSET, QUOTE_ASSET, OrderType.LIMIT, TradeType.BUY, 1, 1)
        self.assertGreater(limit_fee.percent, 0)
        self.assertEqual(len(limit_fee.flat_fees), 0)
        market_fee: TradeFee = self.market.get_fee(BASE_ASSET, QUOTE_ASSET, OrderType.MARKET, TradeType.BUY, 1)
        self.assertGreater(market_fee.percent, 0)
        self.assertEqual(len(market_fee.flat_fees), 0)

    def test_fee_overrides_config(self):
        fee_overrides_config_map["moonx_taker_fee"].value = None
        fee_overrides_config_map["moonx_maker_fee"].value = Decimal('0.5')
        maker_fee: TradeFee = self.market.get_fee("USDT", "ETH", OrderType.LIMIT, TradeType.BUY, Decimal(1),
                                                  Decimal('0.1'))
    ...
    def test_limit_buy(self):
        self.assertGreater(self.market.get_balance(QUOTE_ASSET), 6)
        trading_pair = PAIR
        ...
        quote_amount_traded = Decimal(sum(Decimal(t.amount) * t.price for t in trade_events))
    ...
    def test_limit_sell(self):
        self.assertGreater(self.market.get_balance(BASE_ASSET), 0.02)
        trading_pair = PAIR
        ...
        trade_events = [t for t in self.market_logger.event_log if
                        isinstance(t, OrderFilledEvent) and t.amount is not None]
        base_amount_traded = Decimal(sum(t.amount for t in trade_events))
        quote_amount_traded = Decimal(sum(Decimal(t.amount) * t.price for t in trade_events))
    ...
    def underpriced_limit_buy(self):
        self.assertGreater(self.market.get_balance(QUOTE_ASSET), 4)
        trading_pair = PAIR

        current_bid_price: Decimal = self.market.get_price(trading_pair, True)
        bid_price: Decimal = current_bid_price * Decimal('0.95')
        quantized_bid_price: Decimal = self.market.quantize_order_price(trading_pair, bid_price)
    ...
    def test_order_saving_and_restoration(self):
        ...
        try:
            ...
            for event_tag in self.events:
                self.market.remove_listener(event_tag, self.market_logger)
            self.market: MoonxMarket = MoonxMarket(
                conf.moonx_business_num,
                conf.moonx_api_secret,
                trading_pairs=[PAIR]
            )
